...
class DomScanner {
    ...
    constructor(test: (node: Node) => boolean, addCallback: (node: Node) => void, removeCallback: (node: Node) => void) {
        this.#test = test
        ...
        const walker = document.createTreeWalker(
            ...
            NodeFilter.SHOW_TEXT,
            {
                acceptNode: (node: Node) => {
                    return this.#test(node) ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_REJECT;
                }
            }
        )
