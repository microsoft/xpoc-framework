...
export class FetchError extends Error {
    constructor(public code: FetchErrors, message?: string) {
        super(message);
...
function determineTimeout(timeout?: number): number {
    ...
    if (timeout == null) {
        ...
        if (isNaN(parsed)) {
            console.warn(`Invalid HTTP_TIMEOUT. Env var DOWNLOAD_TIMEOUT cannot be parsed as a number. Using default of ${DEFAULT_TIMEOUT}ms.`)
            return DEFAULT_TIMEOUT
        }
    ...
    if (typeof timeout === 'number') {
        if (timeout < MIN_TIMEOUT || timeout > MAX_TIMEOUT) {
            console.warn(`Invalid HTTP_TIMEOUT. Value of ${timeout}ms, timeout should be between ${MIN_TIMEOUT} and ${MAX_TIMEOUT}ms. Using default of ${DEFAULT_TIMEOUT}ms`)
            return DEFAULT_TIMEOUT
        }
        return timeout
    }
    ...
    console.warn(`Invalid HTTP_TIMEOUT. Value of '${timeout}', timeout should be between ${MIN_TIMEOUT} and ${MAX_TIMEOUT}ms. Using default of ${DEFAULT_TIMEOUT}ms`)
    return DEFAULT_TIMEOUT
}
 ...
 */
export async function fetchObject<T>(url: string, options: RequestInit = {}, timeout?: number): Promise<T | FetchError> {
    ...
    if (response instanceof Error) {
        ...
        if (response.name === 'AbortError') {
            return new FetchError(FetchErrors.TIMEOUT, `Client timeout of ${timeout}ms to ${url}`)
        }
        ...
        if (response instanceof TypeError) {
            ...
            const noCorsFetch = await fetchWithTimeout(url, { ...options, mode: 'no-cors' }, timeout)
            // it failed again, so it's not a cors error. Return the original error
            if (noCorsFetch instanceof Error) {
                return new FetchError(FetchErrors.NETWORK, `Non existent URL ${url} or Network error`)
            }
    ...
    if (response.ok === false) {
        ...
        if (response.status === 404 /* not found */) {
            return new FetchError(FetchErrors.NOT_FOUND, `URL ${url} not found`)
        }

        if (response.status >= 400 && response.status < 500) {
            return new FetchError(FetchErrors.CLIENT, `Fetch client error ${response.status}`)
        }

        if (response.status === 504 /* gateway timeout */) {
            return new FetchError(FetchErrors.TIMEOUT, `Server timeout of ${Math.floor((Date.now() - start) / 100) * 100} ms to ${url}`)
        }

        if (response.status >= 500 && response.status < 600) {
            return new FetchError(FetchErrors.SERVER, `Fetch server error ${response.status}`)
        }
    }
 ...
 */
async function fetchWithTimeout(url: string, options: RequestInit = {}, timeout: number): Promise<Response | Error> {
