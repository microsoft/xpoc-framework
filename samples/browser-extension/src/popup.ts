    ...
    tabs.forEach((tab) => {
        tab.addEventListener('click', () => {
            ...
            if (tabContentId === 'options') {
                ...
                console.log(
                    'originSource obtained in options tab',
                    originSource,
                );
                if (originSource) {
...
const autoVerifyXpocUris = document.getElementById(
    'auto-verify-xpoc-uri-toggle',
) as HTMLInputElement;
...
const originInfoInput = document.getElementById(
    'origin-data-source-input',
) as HTMLInputElement;
...
function displayOriginDataSourceInfo(source: OriginSource) {
    ...
    const sourceInfo = document.getElementById(
        'origin-data-source-info',
    ) as HTMLDivElement;
 ...
 */
async function getXpocResultsForCurrentTab(): Promise<
    { [xpocUri: string]: lookupXpocUriResult }[]
> {
    const storageObj = ((await getLocalStorage('xpocResults')) as {
...
async function showResults() {
    ...
    if (xpocResults.length > 0) {
        ...
        const xpocResult = Object.values(
            xpocResults[0],
        )[0] as lookupXpocUriResult;
        console.log(`xpoc result: ${xpocResult.type}`);
        if (xpocResult.type === 'account' || xpocResult.type === 'content') {
            let account = '';
            let platform = '';
            let prefix = '';
            const baseurl = `https://${xpocResult.baseurl}`;
            const url = `${baseurl}/xpoc-manifest.json`;
    ...
    if (originResults) {
        ...
        if (source) {
            ...
            const pageType =
                originResults.platform === 'Website'
                    ? 'website'
                    : `${originResults.platform} page`;
            resultDiv.innerHTML = `
